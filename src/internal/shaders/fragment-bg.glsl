...
// Adapted from Liquid Glass Studio by Charles Yin.
// Original project: https://github.com/iyinchao/liquid-glass-studio
...
uniform vec3 u_switchOffColor;
uniform vec3 u_switchOnColor;
uniform vec4 u_switchTrackRect;
uniform float u_switchProgress;
...
float roundedRectPxSDF(vec2 p, vec2 center, vec2 size, float radius) {
  vec2 d = abs(p - center) - size * 0.5 + vec2(radius);
  return length(max(d, vec2(0.0))) + min(max(d.x, d.y), 0.0) - radius;
}

vec3 switchTrackColor(vec2 p, vec4 rect, float progress) {
  vec2 local = clamp((p - rect.xy) / rect.zw, vec2(0.0), vec2(1.0));
  vec3 color = mix(u_switchOffColor, u_switchOnColor, progress);
  float topSheen = smoothstep(0.42, 1.0, local.y) * 0.18;
  float bottomShade = smoothstep(0.52, 0.0, local.y) * 0.07;

  color = mix(color, vec3(1.0), topSheen);
  color -= vec3(bottomShade);
...
void main() {
  ...
  vec3 bgColor = vec3(0.082, 0.082, 0.082);
  ...
  if (u_bgType <= 0) {
    ...
    vec2 gridCell = abs(fract(px / 28.0) - 0.5) * 28.0;
    vec2 halfCell = abs(fract((px + 14.0) / 28.0) - 0.5) * 28.0;
    float gridLine = 1.0 - smoothstep(0.28, 0.68, min(gridCell.x, gridCell.y));
    float highlightLine = 1.0 - smoothstep(0.22, 0.58, min(halfCell.x, halfCell.y));

    bgColor = mix(vec3(0.98, 0.99, 1.0), vec3(0.87, 0.91, 0.96), uv.y);
    bgColor = mix(bgColor, vec3(0.12, 0.15, 0.22), gridLine * 0.08);
    bgColor = mix(bgColor, vec3(1.0), highlightLine * 0.07);
  }

  if (u_switchTrackRect.z > 0.0 && u_switchTrackRect.w > 0.0) {
    float trackSdf = roundedRectPxSDF(
      gl_FragCoord.xy,
      u_switchTrackRect.xy + u_switchTrackRect.zw * 0.5,
      u_switchTrackRect.zw,
      u_switchTrackRect.w * 0.5
    );
    float trackMask = 1.0 - smoothstep(-0.25 * u_dpr, 1.25 * u_dpr, trackSdf);
    vec3 trackColor = switchTrackColor(gl_FragCoord.xy, u_switchTrackRect, u_switchProgress);

    bgColor = mix(bgColor, trackColor, trackMask);
  }
