...
uniform float u_reverseLensFactor;
...
vec2 getReverseLensUV(vec2 uv) {
  vec2 centerUv = u_mouseSpring / u_resolution;
...
void main() {
  ...
  if (STEP <= 0) {
    ...
    if (merged < 0.0) {
      vec2 lensUv = getReverseLensUV(v_uv);
      float nmerged = -1.0 * (merged * u_resolution1x.y);
    ...
    float shapeAlpha = 1.0 - smoothstep(-0.001, 0.001, merged);
    outColor.a *= shapeAlpha * u_glassAlpha;
